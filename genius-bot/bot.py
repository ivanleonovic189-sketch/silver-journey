# -*- coding: utf-8 -*-
"""
Genius Routines Bot — напоминания о рутинах развития интуиции/мышления
+ (опционально) утреннее приветствие по первой активности в Telegram.
...
async def send_routine(routine_key: str):
    ...
    await bot.send_message(config.CHAT_ID, r["text"], reply_markup=done_kb(routine_key))


def is_owner(obj) -> bool:
    return obj.from_user is not None and obj.from_user.id == config.CHAT_ID
...
async def cmd_start(msg: Message):
    ...
    lines = "\n".join(
        f"• <b>{r['title']}</b>" for r in ROUTINES.values()
    )
    await msg.answer(
        "🧠 <b>Genius Routines Bot</b>\n\n"
        "Буду напоминать тебе о рутинах:\n" + lines + "\n\n"
        "Команды:\n"
        "/routines — прислать список рутин\n"
        "/note &lt;текст&gt; — записать предчувствие (этап 1)\n"
...
async def cmd_note(msg: Message, command: CommandObject):
    ...
    await msg.answer(f"📝 Предчувствие #{note_id} записано. Отметить исход: /check")
...
async def cmd_check(msg: Message):
    ...
    if not notes:
        return await msg.answer("Непроверенных предчувствий нет. Запиши новое: /note")
...
async def cmd_stats(msg: Message):
    ...
    lines.append("\n<b>Интуиция (этап 1):</b>")
    lines.append(f"• Предчувствий записано: {total}")
...
async def main():
    ...
    if config.WATCHER_ENABLED:
        ...
        tasks.append(run_watcher(bot))
        log.info("Watcher enabled (MTProto)")
    else:
        log.info("Watcher disabled: set API_ID/API_HASH to enable wake detection")
